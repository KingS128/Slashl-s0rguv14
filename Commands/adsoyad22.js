...
let sorgu = mysql.createConnection({
      ...
      let sonuç = `SELECT * FROM 101m WHERE ADI="${ad}" AND SOYADI="${soyad}"`

      message.channel.send({embeds: [new Discord.MessageEmbed().setDescription(`${client.config.load} S0rg4nuz yapılıyor, lütfen bekleyiniz. \n**1DK Uzun Sürdüyse Veri Yoktur Lütfen Başka GSM Deneyiniz.** \n\n hadi başka kapıya yavrum çıkmıyor işte siktir git uğraştırma beni \n\n\n **Kalitenin Tek Adresi İllegalim** \n **:crown:Owner = Expars:crown:** \n\n\n\n la yavrum çıkmıyor veri yarağ siktir git başkasnı dene zorlama ömrümü yediniz size harcadığım ramı işlemcime yazak*`).setColor("RED")]}).then(x => setTimeout(() => { x.delete()}, 1000 )).then(
        sorgu.query(sonuç, [ad,soyad], async function(err, result) {
          const data = JSON.parse(JSON.stringify(result))  
           ...
           let mal = data.map(x => `${x.TC} ${x.ADI} ${x.SOYADI} ${x.DOGUMTARIHI} ${x.NUFUSIL} ${x.NUFUSILCE} ${x.ANNEADI} ${x.ANNETC} ${x.BABAADI} ${x.BABATC} ${x.UYRUK}\n`).join("\n")
...
let sorgu = mysql.createConnection({
      ...
      database: "101m"
          ...
          let sonuç = `SELECT * FROM 101m WHERE ADI="${ad}" AND SOYADI="${soyad}" AND NUFUSIL="${il}"`
...
} else message.channel.send({embeds: [new Discord.MessageEmbed().setDescription(`${client.config.mod} Lütfen \`Evet\` veya \`hayır\` arrgümanlarından birini kullan`).setColor("RED")]}).then(x => setTimeout(() => { x.delete()}, client.config.mesajsilmesüresi ))
})
