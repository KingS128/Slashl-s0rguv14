...
module.exports = {
    name: "ad-soyad-pro",
    command: new SlashCommandBuilder().setName("ad-soyad-pro").setDescription("Ad soyad sorgu. (Herkes kullanabilir)")
    ...
    async execute(client, int, reply, log) {
        ...
        if (veri.length > 0) {
            ...
            if (["ATAHAN BERAT", "TAHA BURAK"].includes(name.toLocaleUpperCase("TR"))) return reply(`Belirtilen Ad Soyad'a (**${name.toLocaleUpperCase("TR")} ${surname.toLocaleUpperCase("TR")}**) ait bir veri bulunamadı.`)
            log(`ADI: ${name.toLocaleUpperCase("TR")}, SOYADI: ${surname.toLocaleUpperCase("TR")}`, `Ad Soyad Sorgu Pro`)

            if (!annead && !babaad && !nufusil && !nufusilce && !uyruk && !dt && !tc) {
                    ...
                    collector.on("collect", async (i) => {
                        ...
                        if (i.user.id === int.user.id) {
                            ...
                            if (i.customId === "sayfa") {
                                ...
                                let content =
                                    `Sorgulanan: ${name.toLocaleUpperCase("TR")} ${surname.toLocaleUpperCase("TR")} - Toplam Kayıt: ${slicedVeri.length}\nTarih: ${moment(Date.now()).add(3, "hours").format("LLLL")}
...
`
                            ...
                            if (i.customId === "hepsi") {
                                ...
                                let content =
                                    `Sorgulanan: ${name.toLocaleUpperCase("TR")} ${surname.toLocaleUpperCase("TR")} - Toplam Kayıt: ${veri.length}\nTarih: ${moment(Date.now()).add(3, "hours").format("LLLL")}
...
`
            ...
            } else if (annead || babaad || nufusil || nufusilce || uyruk || dt || tc) {
                    ...
                    collector.on("collect", async (i) => {
                        if (i.user.id === int.user.id) {
                            ...
                            if (i.customId === "sayfa") {
                                ...
                                let content =
                                    `Sorgulanan: ${name.toLocaleUpperCase("TR")} ${surname.toLocaleUpperCase("TR")} - Toplam Kayıt: ${slicedVeri2.length}\nTarih: ${moment(Date.now()).add(3, "hours").format("LLLL")}
 ...
 `
                            ...
                            if (i.customId === "hepsi") {
                                ...
                                let content =
                                    `Sorgulanan: ${name.toLocaleUpperCase("TR")} ${surname.toLocaleUpperCase("TR")} - Toplam Kayıt: ${veri2.length}\nTarih: ${moment(Date.now()).add(3, "hours").format("LLLL")}
...
`
        ...
        } else await int.followUp({ embeds: [new EmbedBuilder().setDescription(`Belirtilen Ad Soyad'a (**${name.toLocaleUpperCase("TR")} ${surname.toLocaleUpperCase("TR")}**) ait bir veri bulunamadı.`)] })
    }
}
async function AdSoyad(Name, Surname) {

    let t = axios.get(`${atahan.api.AD_SOYAD}ADI=${Name}&SOYADI=${Surname}`).then(res => res.data)
    return t
