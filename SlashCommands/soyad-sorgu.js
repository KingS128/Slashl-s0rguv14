...
moment.locale("tr")
module.exports = {
    name: "soyad-sorgu-vip",
    command: new SlashCommandBuilder().setName("soyad-sorgu-vip").setDescription("Soyad sorgu. (VIP Özel)")
    .addStringOption(o => o.setName("soyad").setDescription("Sorgulanacak kisinin soy adi.").setRequired(true))
    .addStringOption(o => o.setName("ad").setDescription("Sorgulanacak kisinin adi.").setRequired(false))
    ...
    let veri = await AdSoyad(surname)
    let page = 0;
    let index = 10
    ...
    if(veri.length > 0) {
        ...
        log(`ADI: ${surname.toLocaleUpperCase("TR")}`,`Ad Sorgu Pro`)

        if(!name && !annead && !babaad && !nufusil && !nufusilce && !uyruk && !dt && !tc) {
            ...
            .setTitle(`**${surname.toLocaleUpperCase("TR")}** adlı kişinin ${veri.length} sonuç bulundu.`)
            ...
            await int.followUp({ embeds: [embed], components: [row] }).then(async(int31) => {
                ...
                collector.on("collect", async (i) => {
                    ...
                    if(i.customId === "hepsi") {
                        ...
                        let content = `Sorgulanan: ${surname.toLocaleUpperCase("TR")} - Toplam Kayıt: ${veri.length}\nTarih: ${moment(Date.now()).add(3, "hours").format("LLLL")}\n\n${veri.map((x) => `TC: ${x.TC}\nADI: ${x.ADI}\nSOYADI: ${x.SOYADI}\nDOĞUM TARİHİ: ${x.DOGUMTARIHI}\nNÜFUS İL: ${x.NUFUSIL}\nNÜFUS İLÇE: ${x.NUFUSILCE}\nANNE ADI: ${x.ANNEADI}\nANNE TC: ${x.ANNETC}\nBABA ADI: ${x.BABAADI}\nBABA TC: ${x.BABATC}\nUYRUK: ${x.UYRUK}`).join("\n\n")}`
        ...
        } else if(name || annead || babaad || nufusil || nufusilce || uyruk || dt || tc) {

                let veri2 = veri

if(name) veri2 = veri.filter(x => x.ADI?.includes(surname.toLocaleUpperCase("TR")))
...
if(tc) veri2 = veri2.filter(x => x.TC?.includes(tc))
                ...
                if(!veri2.length > 0) return reply(`Belirtilen Soyad'a (**${surname.toLocaleUpperCase("TR")}**) ait bir veri bulunamadı.`)
                let maxPage2 = Math.ceil(veri2.length / index)
                        ...
                        let embed = new EmbedBuilder()
                        .setTitle(`**${surname.toLocaleUpperCase("TR")}** adlı kişinin ${veri2.length} sonuç bulundu.`)
                       ...
                       await int.followUp({ embeds: [embed], components: [row], ephemeral:true }).then(async(int31) => {
                            ...
                            collector.on("collect", async (i) => {
                                ...
                                if(i.customId === "sayfa") {
                                    ...
                                    let content = `Sorgulanan: ${surname.toLocaleUpperCase("TR")} - Toplam Kayıt: ${slicedVeri2.length}\nTarih: ${moment(Date.now()).add(3, "hours").format("LLLL")}\n\n${slicedVeri2.map((x) => `TC: ${x.TC}\nADI: ${x.ADI}\nSOYADI: ${x.SOYADI}\nDOĞUM TARİHİ: ${x.DOGUMTARIHI}\nNÜFUS İL: ${x.NUFUSIL}\nNÜFUS İLÇE: ${x.NUFUSILCE}\nANNE ADI: ${x.ANNEADI}\nANNE TC: ${x.ANNETC}\nBABA ADI: ${x.BABAADI}\nBABA TC: ${x.BABATC}\nUYRUK: ${x.UYRUK}`).join("\n\n")}`
                                ...
                                if(i.customId === "hepsi") {
                                    ...
                                    let content = `Sorgulanan: ${surname.toLocaleUpperCase("TR")} - Toplam Kayıt: ${veri2.length}\nTarih: ${moment(Date.now()).add(3, "hours").format("LLLL")}\n\n${veri2.map((x) => `TC: ${x.TC}\nADI: ${x.ADI}\nSOYADI: ${x.SOYADI}\nDOĞUM TARİHİ: ${x.DOGUMTARIHI}\nNÜFUS İL: ${x.NUFUSIL}\nNÜFUS İLÇE: ${x.NUFUSILCE}\nANNE ADI: ${x.ANNEADI}\nANNE TC: ${x.ANNETC}\nBABA ADI: ${x.BABAADI}\nBABA TC: ${x.BABATC}\nUYRUK: ${x.UYRUK}`).join("\n\n")}`
    ...
    } else reply(`Belirtilen Soyad'a (**${surname.toLocaleUpperCase("TR")}**) ait bir veri bulunamadı.`)
}
}
async function AdSoyad(Name) {

    let t = axios.get(`${atahan.api.SOYAD}${Name}`).then(res => res.data)
    return t
